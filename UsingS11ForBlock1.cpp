...
void using_s11_find_block1(uint32 block[], const uint32 IV[])
{
	...
	std::vector<uint32> q_vec_mac(32);
	for (unsigned k = 0; k < q_vec_mac.size(); ++k)
		q_vec_mac[k] = ((k << 5) ^ (k << 6) ^ (k << 7) ^ (k << 24) ^ (k << 27)) & 0x880002a0;
	
	std::vector<uint32> q_vec_mac_9(1 << 9);
	for (unsigned k = 0; k < q_vec_mac_9.size(); ++k)
		q_vec_mac_9[k] = ((k << 1) ^ (k << 3) ^ (k << 8) ^ (k << 12) ^ (k << 15) ^ (k << 18)) & 0x04710c12;
	
	while (1) 
	{
		uint32 aa = Q[FLAG] & 0x80000000;

		Q[FLAG + 2] = (changing() & 0x75bef63e) | 0x0a410041 | aa;
		Q[FLAG + 3] = (changing() & 0x10345614) | 0x0202a9e1 | (Q[FLAG + 2] & 0x84000002);
		Q[FLAG + 4] = (changing() & 0x00145400) | 0xe84ba909 | (Q[FLAG + 3] & 0x00000014);
		Q[FLAG + 5] = (changing() & 0x80000000) | 0x75e90b1d | (Q[FLAG + 4] & 0x00145400);
		Q[FLAG + 6] = 0x7c23ff5a | (Q[FLAG + 5] & 0x80000000);
		Q[FLAG + 7] = (changing() & 0x40000880) | 0x114bf41a;
		Q[FLAG + 8] = (changing() & 0x00002090) | 0xb352dd01;
		Q[FLAG + 9] = (changing() & 0x00044000) | 0x7a803124;
		Q[FLAG +10] = (changing() & 0x00002000) | 0xf28a92c9 | (Q[FLAG + 9] & 0x00044000);
		Q[FLAG +11] = (changing() & 0x128a8108) | 0xc5710ed7 | (Q[FLAG + 10] & 0x00002000);
		Q[FLAG +12] = (changing() & 0x9edb8d7f) | 0x20003080 | (~Q[FLAG + 11] & 0x00200000);
		Q[FLAG +13] = (changing() & 0x3efb1d77) | 0x4004c008 | (Q[FLAG + 12] & 0x80000000);
		Q[FLAG +14] = (changing() & 0x1fff5d77) | 0x0000a288;
		Q[FLAG +15] = (changing() & 0x1efe7ff7) | 0x20008000 | (~Q[FLAG + 14] & 0x00010000);
		Q[FLAG +16] = (changing() & 0x1ffdffff) | 0x20000000 | (~Q[FLAG + 15] & 0x40020000);
		...
		const uint32 q1a = 0x02000861 ^ (Q[FLAG + 0] & 0x80000020);
		...
		while (counter < (1 << 12))
		{
			...
			uint32 q1 = q1a | (changing() & 0x7dfff79e);
		...
		for (unsigned k10 = 0; k10 < 32; ++k10)
		{
			uint32 q10 = q10b | (q_vec_mac[k10]&0x08000040);
			uint32 m10 = RR(Q[FLAG+11]-q10,17);
			uint32 q9 = q9b | (q_vec_mac[k10]&0x80000280);
