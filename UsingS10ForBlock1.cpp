...
void using_s10_find_block1(uint32 block[], const uint32 IV[])
{
	...
	std::vector<uint32> q_vec_mac(16);
	for (unsigned k = 0; k < q_vec_mac.size(); ++k)
		q_vec_mac[k] = ((k<<2) ^ (k<<8) ^ (k<<11) ^ (k<<25)) & 0x08004204;
	
	std::vector<uint32> q_vec_mac_9(1<<10);
	for (unsigned k = 0; k < q_vec_mac_9.size(); ++k)
		q_vec_mac_9[k] = ((k<<1) ^ (k<<2) ^ (k<<3) ^ (k<<7) ^ (k<<12) ^ (k<<15) ^ (k<<18) ^ (k<<20)) & 0x2471042a;
	
	while (true) 
	{
		uint32 aa = Q[FLAG] & 0x80000000;

		Q[FLAG + 2] = (changing() & 0x79b0c6ba) | 0x024c3841 | aa;
		Q[FLAG + 3] = (changing() & 0x19300210) | 0x2603096d | (Q[FLAG + 2] & 0x80000082);
		Q[FLAG + 4] = (changing() & 0x10300000) | 0xe4cae30c | (Q[FLAG + 3] & 0x01000030);
		Q[FLAG + 5] = (changing() & 0x10000000) | 0x63494061 | (Q[FLAG + 4] & 0x00300000);
		Q[FLAG + 6] = 0x7deaff68;
		Q[FLAG + 7] = (changing() & 0x20444000) | 0x09091ee0;
		Q[FLAG + 8] = (changing() & 0x09040000) | 0xb2529f6d;
		Q[FLAG + 9] = (changing() & 0x00040000) | 0x10885184;
		Q[FLAG + 10] = (changing() & 0x00000080) | 0x428afb11 | (Q[FLAG + 9] & 0x00040000);
		Q[FLAG + 11] = (changing() & 0x128a8110) | 0x6571266b | (Q[FLAG + 10] & 0x0000080);
		Q[FLAG + 12] = (changing() & 0x3ef38d7f) | 0x00003080 | (~Q[FLAG + 11] & 0x00080000);
		Q[FLAG + 13] = (changing() & 0x3efb1d77) | 0x0004c008;
		Q[FLAG + 14] = (changing() & 0x5fff5d77) | 0x8000a288;
		Q[FLAG + 15] = (changing() & 0x1efe7ff7) | 0xe0008000 | (~Q[FLAG + 14] & 0x00010000);
		Q[FLAG + 16] = (changing() & 0x5ffdffff) | 0x20000000 | (~Q[FLAG + 15] & 0x00020000);
		//std::cout << "ennan" << std::endl;
		...
		//std::cout << tt17 << std::endl;
		//std::cout << tt18 << std::endl;
		//std::cout << tt19 << std::endl;
		
		const uint32 tt0 = FF(Q[FLAG + 0], Q[FLAG - 1], Q[FLAG - 2]) + Q[FLAG - 3] + 0xd76aa478;
		const uint32 tt1 = Q[FLAG - 2] + 0xe8c7b756;		

		const uint32 q1a = 0x02000941 ^ (Q[FLAG + 0] & 0x80000000);
		...
		while (counter < (1 << 12))
		{
			...
			uint32 q1 = q1a | (changing() & 0x7dfdf6be);
		...
		for (unsigned k10 = 0; k10 < 16; ++k10)
		{
			uint32 q10 = q10b | (q_vec_mac[k10]&0x08000004);
			uint32 m10 = RR(Q[FLAG+11]-q10,17);
			uint32 q9 = q9b | (q_vec_mac[k10]&0x00004200);
