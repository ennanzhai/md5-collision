...
void myusing_Orignal_find_block1_fun(uint32 block[], const uint32 IV[])
{
	...
	std::vector<uint32> q_vec_mac_4(1 << 6);

	for (unsigned k = 0; k < q_vec_mac_4.size(); ++k)
		q_vec_mac_4[k] = ((k << 13) ^ (k << 19)) & 0x01c0e000;

	std::vector<uint32> q_vec_mac_9(1 << 5), q10mask(1 << 5);
	for (unsigned k = 0; k < q_vec_mac_9.size(); ++k)
	{
		uint32 msk = (k<<5) ^ (k<<13) ^ (k<<17) ^ (k<<24);
		q_vec_mac_9[k] = msk &  0x00084000;
		q10mask[k] = msk & 0x18000020;
	}
	
	std::vector<uint32> q_vec_mac_92(1 << 10);
	for (unsigned k = 0; k < q_vec_mac_92.size(); ++k)
		q_vec_mac_92[k] = ((k << 1) ^ (k << 7) ^ (k << 14) ^ (k << 15) ^ (k << 22)) & 0x6074041c;
	...
	while (1) 
	{
		...
		Q[FLAG + 2] = (changing() & 0x71de7799) | 0x0c008840 | bb;
		Q[FLAG + 3] = (changing() & 0x01c06601) | 0x3e1f0966 | (Q[FLAG + 2] & 0x80000018);
		Q[FLAG + 4] = 0x3a040010 | (Q[FLAG + 3] & 0x80000601);
		Q[FLAG + 5] = (changing() & 0x03c0e000) | 0x482f0e50 | aa;
		Q[FLAG + 6] = (changing() & 0x600c0000) | 0x05e2ec56 | aa;
		Q[FLAG + 7] = (changing() & 0x604c203e) | 0x16819e01 | bb | (Q[FLAG + 6] & 0x01000000);
		Q[FLAG + 8] = (changing() & 0x604c7c1c) | 0x043283e0 | (Q[FLAG + 7] & 0x80000002);
		Q[FLAG + 9] =  (changing() & 0x00002800) | 0x1c0101c1 | (Q[FLAG + 8] & 0x80001000);
		Q[FLAG + 10] = 0x078bcbc0 | bb;
		Q[FLAG + 11] = (changing() & 0x07800000) | 0x607dc7df | bb;
		Q[FLAG + 12] = (changing() & 0x00f00f7f) | 0x00081080 | (Q[FLAG + 11] & 0xe7000000);
		Q[FLAG + 13] = (changing() & 0x00701f77) | 0x3f0fe008 | aa;
		Q[FLAG + 14] = (changing() & 0x00701f77) | 0x408be088 | aa;
		Q[FLAG + 15] = (changing() & 0x00ff3ff7) | 0x7d000000;
		Q[FLAG + 16] = (changing() & 0x4ffdffff) | 0x20000000 | (~Q[FLAG + 15] & 0x00020000);
		...
		const uint32 q1a = 0x04200040 | (Q[FLAG + 2] & 0xf01e1080);
		...
		while (counter < (1 << 12))
		{
			...
			uint32 q1 = q1a | (changing() & 0x01c0e71f);
		...
		while (counter < (1<<6))
		{
			Q[FLAG + 4] = q4b ^ q_vec_mac_4[counter];
			++counter;
			MD5_REVERSE_STEP(5, 0x4787c62a, 12);
			uint32 q21 = tt21 + block[5];
			q21 = RL(q21, 5); q21 += Q[FLAG+20];
			if (0 != ((q21^Q[FLAG+20]) & 0x80020000)) 
				continue;
			...
			while (counter2 < (1 << 5))
			{
				...
				uint32 q9 = q9b ^ q_vec_mac_9[counter2];
				++counter2;
