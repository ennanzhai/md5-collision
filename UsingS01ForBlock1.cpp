...
void using_s01_find_block1(uint32 block[], const uint32 IV[])
{
	...
	for (unsigned k = 0; k < q_vec_mac.size(); ++k)
		q_vec_mac[k] = ((k << 4) ^ (k << 11) ^ (k << 24) ^ (k << 27)) & 0x88002030;
	
	std::vector<uint32> q_vec_mac_9(1<<9);
	for (unsigned k = 0; k < q_vec_mac_9.size(); ++k)
		q_vec_mac_9[k] = ((k << 1) ^ (k << 7) ^ (k << 9) ^ (k << 12) ^ (k << 15) ^ (k << 19) ^ (k << 22)) & 0x44310d02;
	
	while (1) 
	{
		uint32 aa = Q[FLAG] & 0x80000000;

		Q[FLAG + 2] = (changing() & 0x4db0e03e) | 0x32460441 | aa;
		Q[FLAG + 3] = (changing() & 0x0c000008) | 0x123c3af1 | (Q[FLAG + 2] & 0x80800002);
		Q[FLAG + 4] = 0xe398f812 ^ (Q[FLAG + 3] & 0x88000000);
		Q[FLAG + 5] = (changing() & 0x82000000) | 0x4c66e99e;
		Q[FLAG + 6] = (changing() & 0x80000000) | 0x27180590;
		Q[FLAG + 7] = (changing() & 0x00010130) | 0x51ea9e47;
		Q[FLAG + 8] = (changing() & 0x40200800) | 0xb7c291e5;
		Q[FLAG + 9] = (changing() & 0x00044000) | 0x380002b4;
		Q[FLAG +10] = 0xb282b208 | (Q[FLAG + 9] & 0x00044000);
		Q[FLAG +11] = (changing() & 0x12808008) | 0xc5712f47;
		Q[FLAG +12] = (changing() & 0x1ef18d7f) | 0x000a3080;
		Q[FLAG +13] = (changing() & 0x1efb1d77) | 0x4004c008;
		Q[FLAG +14] = (changing() & 0x1fff5d77) | 0x6000a288;
		Q[FLAG +15] = (changing() & 0x1efe7ff7) | 0xa0008000 | (~Q[FLAG + 14] & 0x00010000);
		Q[FLAG +16] = (changing() & 0x1ffdffff) | 0x20000000 | (~Q[FLAG + 15] & 0x00020000);
		...
		const uint32 q1a = 0x02000021 ^ (Q[FLAG + 0] & 0x80000020);
		...
		while (counter < (1 << 12))
		{
			...
			uint32 q1 = q1a | (changing() & 0x7dfff39e);
		...
		{
			uint32 q10 = q10b | (q_vec_mac[k10]&0x08000030);
			uint32 m10 = RR(Q[FLAG+11]-q10,17);
			uint32 q9 = q9b | (q_vec_mac[k10]&0x80002000);
